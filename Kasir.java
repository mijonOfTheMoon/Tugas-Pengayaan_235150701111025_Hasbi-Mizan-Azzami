...
public class Kasir {
    ...
    public void tampilkanDaftarMeja() {
        ...
        for (int i = 0; i < this.daftarMeja.length; i++) {
            System.out.println("Meja " + (i + 1) + (daftarMeja[i].isKosong() ? " (kosong)"
                    : " (terisi oleh pelanggan " + daftarMeja[i].getPelanggan().getNama() + ")"));
        }
    }
